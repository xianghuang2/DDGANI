...
def data_loader(file_name, miss_rate, categorical_cols, label, miss_type, miss_seed, run_num):
    ...
    if data_x.shape[0] > (run_num+1)*500:
        data_x = data_x.iloc[run_num*500:(run_num+1)*500,:]
    ...
    # MAX_N = 400   # 🔴 TabImpute 安全范围
    # start = run_num * MAX_N
    # end = start + MAX_N
    # if start < len(data_x):
    #     data_x = data_x.iloc[start:end]
    # else:
    #     data_x = data_x.sample(n=MAX_N, random_state=miss_seed)
    # over
    ...
    while flag:
        if miss_type == "MCAR":
            ...
            # data_m = binary_sampler_col(1 - miss_rate, no, dim, [1])
        elif miss_type == "MAR":
...
def check_column_for_nans_and_one_value(arr):
    # 遍历每一列
    for col in arr.T:  # arr.T 是转置后的数组，便于按列迭代
        unique_values = np.unique(col)  # 获取该列的唯一值
        if len(unique_values) == 2 and np.isnan(unique_values[0]) and not np.isnan(unique_values[1]):
            # 如果唯一值有两个，其中一个是NaN，另一个是非NaN值
            return True
