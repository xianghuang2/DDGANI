...
def data_loader(file_name, miss_rate, categorical_cols, label, miss_type, k):
    ...
    n = len(data_x)
    train_data, val_data = train_test_split(data_x, test_size=0.2, shuffle=False)
    # data_x = train_data
    ...
    return data_x, miss_data_x, data_m, pd.DataFrame(label_column), true_data, con_cols, nan_data, val_data
