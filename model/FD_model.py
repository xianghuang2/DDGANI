...
def set_fd_tau_from_param(param):
    """Set FD_TAU from param dictionary, default to 3."""
    global FD_TAU
    if isinstance(param, dict) and 'cfd_lhs_max_size' in param:
        try:
            FD_TAU = int(param['cfd_lhs_max_size'])
...
def get_FD_model_Tree_GroundTruth(miss_data, data_m, categorical_cols, zero_feed_data, fields, device, sort_corr_dict,data_name):
    ...
    # Get non-missing positions for all attributes
    has_data_index = []
    observer_data = []
    ...
    encoder_data = []
    # Only consider correlation of categorical attributes
    new_sort_corr_dict = {}
    for col_index, col_val in enumerate(data.T):
        arr = np.array(col_val)
        cur_corr_sort = []
        # Get unique categorical values and map them to integers
    ...
    # Build decision tree for each categorical data
    col_tree = {}
    if data_name == 'adult':
    ...
    for key, value in col_tree.items():
        if len(value) > 0:
            ...
            for x_index in x_index_list:
                ...
                # Pass x_index, y_index, zero_feed_data, has_data_index, fields, output model
                model = get_model_by_tree(x_index, y_index, zero_feed_data, has_data_index, fields, device)
                if model is not None:
                    models.append(model)
    print("Total FD count: {}\nSpecific FDs as follows:".format(len(FD_list)))
    for fd in FD_list:
        print(fd)
    return models
...
def get_FD_model_Tree_GroundTruth_From_Complete_Data(ori_data, categorical_cols, zero_feed_data, fields, device, sort_corr_dict, param):
    """
    Mine Ground Truth FD from complete data
    Parameters:
    - ori_data: Complete original data (pandas.DataFrame)
    - categorical_cols: List of categorical column indices
    - zero_feed_data: Zero-filled data (tensor)
    - fields: Field information
    - device: Device
    - sort_corr_dict: Correlation sorting dictionary
    ...
    # For complete data, all rows have data
...
def get_FD_model_Tree(miss_data, data_m, categorical_cols, zero_feed_data, fields, device, sort_corr_dict,param):
    # Set FD_TAU from param
    set_fd_tau_from_param(param)
...
def get_CFD_model_Tree(miss_data,data_m,categorical_cols,zero_feed_data,fields,device,sort_corr_dict,param,min_rows=5,max_pattern_size=None,col_idx_to_unique_values=None):
    """
    CFD version of FD mining - almost identical to get_FD_model_Tree but with pattern enumeration.
    Discovers Conditional Functional Dependencies (Z → A | pattern) where pattern is generated
    from Z's categorical value space.

    The logic is almost identical to FD mining, but after enumerating each value,
    we check if FD holds on the pattern corresponding to itself, with min_rows judgment.

    Returns: models, dict_A, dict_B, dict_cnt
    - dict_A: key=(LHS, RHS), value=pattern (high-quality CFD)
    - dict_B: key=(LHS, RHS), value=list of (pattern, support) sorted by support descending (low-quality CFD)
    - dict_cnt: key=(LHS, RHS), value=number of CFDs for this structure

    Parameters:
    - col_idx_to_unique_values: dict mapping column index to unique values array for decoding
    """
    # Set FD_TAU from param
    set_fd_tau_from_param(param)
    ...
    _col_idx_to_unique_values = {} if col_idx_to_unique_values is None else col_idx_to_unique_values.copy()
    ...
    for col_index, col_val in enumerate(data.T):
        ...
        if col_index in categorical_cols:
            ...
            value_cat.append(miss_data.columns[col_index])
            # Store unique values for decoding
            _col_idx_to_unique_values[col_index] = unique_values
        else:
            bins = np.array_split(np.sort(arr), 10)
            encoded_arr = np.zeros_like(arr)
            for i, bin_ in enumerate(bins):
        ...
        new_sort_corr_dict[col_index] = cur_corr_sort
        encoder_data.append(encoded_arr)
    encoder_data = np.array(encoder_data, dtype=np.int32)
    col_tree = {}
    for col_name in value_cat:
        if col_name == "state":
            print(1)
        ...
        for node in new_sort_corr_dict[col_dict[col_name]]:
            graph[node] = new_sort_corr_dict[col_dict[col_name]][new_sort_corr_dict[col_dict[col_name]].index(node) + 1:]
        buildCTANE_CFD(encoder_data, col_dict, col_name, has_data_index, featLabels, choose_row_index, graph, min_rows)
        col_tree[col_name] = featLabels
    models = []
    CFD_list = []
    dict_A = {}  # key=(LHS, RHS), value=pattern (high-quality CFD)
    ...
    for key, value in col_tree.items():
        if len(value) > 0:
            ...
            for x_index in x_index_list:
                new_x_index = [values[i] for i in x_index]
                # For each discovered FD structure, enumerate patterns on LHS + RHS attributes
                pattern_results = enumerate_patterns_for_cfd(x_index, y_index, encoder_data, col_dict, has_data_index, miss_data, min_rows)
                if pattern_results is not None:
                    for pattern_list, support in pattern_results:
                        # Build complete pattern string with wildcards
                        entries = []
                        for i, attr_idx in enumerate(x_index):
                            val = pattern_list[i]
                            if val != '_':
                                # Decode the encoded value back to original
                                if attr_idx in _col_idx_to_unique_values:
                                    unique_vals = _col_idx_to_unique_values[attr_idx]
                                    decoded_val = unique_vals[int(val) - 1]
                                    entries.append(f"{values[attr_idx]}={decoded_val}")
                                else:
                                    entries.append(f"{values[attr_idx]}={val}")
                            else:
                                entries.append(f"{values[attr_idx]}=_")
                        # RHS attribute
                        rhs_val = pattern_list[-1]
                        if rhs_val != '_':
                            # Decode the encoded value back to original
                            if y_index in _col_idx_to_unique_values:
                                unique_vals = _col_idx_to_unique_values[y_index]
                                decoded_val = unique_vals[int(rhs_val) - 1]
                                entries.append(f"{values[y_index]}={decoded_val}")
                            else:
                                entries.append(f"{values[y_index]}={rhs_val}")
                        else:
                            entries.append(f"{values[y_index]}=_")

                        pattern_str = ",".join(entries)
                        CFD_list.append('{}---->{} | {}'.format(new_x_index, values[y_index], pattern_str))

                        # Populate dict_A and dict_B based on support
                        structure_key = (tuple(x_index), y_index)
                        if support >= min_rows:
                            # High quality CFD, store in dict_A
                            dict_A[structure_key] = pattern_list
                        else:
                            # Low quality CFD, store in dict_B
                        ...
                        model = get_CFD_model_by_tree(x_index, y_index, pattern_list, zero_feed_data, has_data_index, fields, device)
                        if model is not None:
                            models.append(model)
    print("Total CFD count: {}\nSpecific CFDs as follows:".format(len(CFD_list)))
    for cfd in CFD_list:
        print(cfd)

    with open('out/CFD_list/{}.txt'.format(param['name']), 'w') as f:
        for cfd in CFD_list:
            f.write(cfd + '\n')
...
def get_CFD_GroundTruth_model_Tree(ori_data, categorical_cols, zero_feed_data, fields, device, sort_corr_dict, param, min_rows, max_pattern_size=None, col_idx_to_unique_values=None):
    """
    ...
    # train models and collect ground-truth CFDs
    models = []
    CFD_list = []
    dict_A = {}  # key=(LHS, RHS), value=pattern (high-quality CFD)
    dict_B = {}  # key=(LHS, RHS), value=list of (pattern, support) sorted by support descending (low-quality CFD)
    ...
    for key, value in col_tree.items():
        if len(value) > 0:
            ...
            for x_index in x_index_list:
                new_x_index = [values[i] for i in x_index]
                # For each discovered FD structure, enumerate patterns on LHS + RHS attributes
                pattern_results = enumerate_patterns_for_cfd(x_index, y_index, encoder_data, col_dict, has_data_index, ori_data, min_rows)
                if pattern_results is not None:
                    for pattern_list, support in pattern_results:
                        # Expand pattern to include all LHS attributes (in x_index order) and RHS.
                        x_names = [values[i] for i in x_index]
                        entries = []
                        lhs_all_wildcard = True
                        for k in x_index:
                            val = pattern_list[x_index.index(k)]
                            if val != '_':
                                # Decode the encoded value back to original
                                if k in _col_idx_to_unique_values:
                                    unique_vals = _col_idx_to_unique_values[k]
                                    decoded_val = unique_vals[int(val) - 1]
                                    entries.append(f"{values[k]}={decoded_val}")
                                else:
                                    entries.append(f"{values[k]}={val}")
                                lhs_all_wildcard = False
                            else:
                                entries.append(f"{values[k]}=_")
                        rhs_val = pattern_list[-1] if len(pattern_list) > len(x_index) else "_"
                        if rhs_val != '_':
                            # Decode the encoded value back to original
                            if y_index in _col_idx_to_unique_values:
                                unique_vals = _col_idx_to_unique_values[y_index]
                        ...
                        if lhs_all_wildcard and sum(1 for p in pattern_list[:-1] if p != '_') > 0:
                            continue
                        pattern_str = ",".join(entries)
    ...
    # Note: constant-case discovery removed per configuration (no special ∅->A|tp handling)

    # output
    out_name = param['name'] if isinstance(param, dict) and 'name' in param else 'groundtruth'
    print(f"GT: Total CFD count: {len(CFD_list)}\nSpecific CFDs as follows:")
    for cfd in CFD_list:
        print(cfd)
...
def get_CFD_model_by_tree(lhs, rhs, pattern, zero_feed_data, has_data_index, fields, device):
    ...
    # Train model (if there is training data)
    if len(set_index) > 0:
        X = []
        for x in lhs:
            cur_data = zero_feed_data[set_index]
...
def buildTreeDFS(encoder_data, col_dict, col_name, has_data_index, featLabels, cur_choose_row_index, graph):
    # print(f"Start mining attribute '{col_name}'  functional dependencies...")
    for node in graph.keys():
        cur_node = []
        DFS(encoder_data, col_dict, col_name, has_data_index, featLabels, cur_choose_row_index, graph, cur_node, node)
    # print(f"Attribute '{col_name}' mining completed, found {len(featLabels)} FD combinations")
...
def DFS(encoder_data, col_dict, col_name, has_data_index, featLabels, cur_choose_row_index, graph, cur_node, start_node):
    ...
    if isCurNodeFD(encoder_data, col_dict, col_name, has_data_index, cur_node):
        ...
        # print(f"Found FD: {left_attrs} --> {col_name}")
...
def buildCTANE_CFD(encoder_data, col_dict, col_name, has_data_index, featLabels, choose_row_index, graph, min_rows):
    """
    CFD version of buildTreeDFS - almost identical to FD mining but with pattern enumeration.
    After enumerating each value, check if FD holds on the pattern corresponding to itself.
    """
    for node in graph.keys():
        cur_node = []
        DFS_CFD(encoder_data, col_dict, col_name, has_data_index, featLabels, choose_row_index, graph, cur_node, node, min_rows)


def DFS_CFD(encoder_data, col_dict, col_name, has_data_index, featLabels, cur_choose_row_index, graph, cur_node, start_node, min_rows):
    """
    CFD version of DFS - discover LHS structures that have conditional FDs, not just basic FDs.
    """
    cur_node.append(start_node)
    if isCurNodeInFDs(cur_node, featLabels):
        return
    ...
    # For CFD, always try to enumerate patterns, regardless of whether basic FD holds
    pattern_results = enumerate_patterns_for_cfd_simple(cur_node, col_dict[col_name], encoder_data, col_dict, has_data_index, cur_choose_row_index, min_rows)
    if pattern_results is not None and len(pattern_results) > 0:
        # If we found any valid patterns (including conditional ones), record this LHS structure
        new_node = cur_node.copy()
        updateFeatLabels(new_node, featLabels)

    # Always continue searching for larger LHS structures
    for node in graph[start_node]:
        if node not in cur_node:
            start_node = node
            DFS_CFD(encoder_data, col_dict, col_name, has_data_index, featLabels, cur_choose_row_index, graph, cur_node, start_node, min_rows)
            cur_node.pop()


def enumerate_patterns_for_cfd_simple(lhs_indices, rhs_index, encoder_data, col_dict, has_data_index, row_subset, min_rows):
    """
    Comprehensive pattern enumeration for CFD - discover all patterns where FD holds.
    Uses level-wise enumeration to find patterns of increasing complexity.
    """
    # Get all attributes involved (LHS + RHS)
    all_attrs = lhs_indices + [rhs_index]

    # Find rows where all attributes are observed
    valid_rows = row_subset
    for attr_idx in all_attrs:
        valid_rows = np.intersect1d(valid_rows, has_data_index[attr_idx])

    if len(valid_rows) < min_rows:
        return None

    # Get column name for RHS
    rhs_col_name = None
    for name, idx in col_dict.items():
        if idx == rhs_index:
            rhs_col_name = name
            break

    if rhs_col_name is None:
        return None

    valid_patterns = []

    # Level 0: Empty pattern (basic FD)
    if isCurNodeFD_InRows(encoder_data, col_dict, rhs_col_name, has_data_index, lhs_indices, valid_rows):
        pattern = ['_'] * (len(lhs_indices) + 1)
        valid_patterns.append((pattern, len(valid_rows)))
        # If basic FD holds, no need to enumerate conditional patterns
        return valid_patterns

    # Level 1: Single attribute patterns
    pattern_attrs = lhs_indices + [rhs_index]  # Include RHS for pattern constraints

    for i, attr in enumerate(pattern_attrs):
        unique_vals = np.unique(encoder_data[attr][valid_rows])
        # Limit the number of values to check (take top 50% by frequency)
        if len(unique_vals) > 1:
            # Count frequency for all values
            val_counts = {}
            for val in unique_vals:
                count = np.sum(encoder_data[attr][valid_rows] == val)
            ...
            num_to_take = max(1, int(len(sorted_vals) * 0.5))  # At least 1
            unique_vals = sorted_vals[:num_to_take]

        for val in unique_vals:
            pattern_rows = valid_rows[encoder_data[attr][valid_rows] == val]
            if len(pattern_rows) >= min_rows:
                if isCurNodeFD_InRows(encoder_data, col_dict, rhs_col_name, has_data_index, lhs_indices, pattern_rows):
                    # Build pattern
                    pattern = ['_'] * (len(lhs_indices) + 1)
                    if i < len(lhs_indices):  # LHS attribute
                        pattern[i] = str(val)
                    else:  # RHS attribute
                        pattern[-1] = str(val)
                    valid_patterns.append((pattern, len(pattern_rows)))

    # Level 2: Two attribute patterns (only for LHS attributes)
    if len(lhs_indices) >= 2:
        for i in range(len(lhs_indices)):
            for j in range(i + 1, len(lhs_indices)):
                attr1, attr2 = lhs_indices[i], lhs_indices[j]

                # Get most frequent value combinations
                val_pairs = {}
                for row_idx in valid_rows:
                    val1 = encoder_data[attr1][row_idx]
                    val2 = encoder_data[attr2][row_idx]
                    key = (val1, val2)
                    val_pairs[key] = val_pairs.get(key, 0) + 1

                # Take top 50% combinations by frequency
                if val_pairs:
                    sorted_pairs = sorted(val_pairs.items(), key=lambda x: x[1], reverse=True)
                    num_to_take = max(1, int(len(sorted_pairs) * 0.5))  # At least 1
                    top_pairs = sorted_pairs[:num_to_take]
                else:
                    top_pairs = []

                for (val1, val2), count in top_pairs:
                    if count < min_rows:
                        continue

                    pattern_rows = valid_rows[
                        (encoder_data[attr1][valid_rows] == val1) &
                        (encoder_data[attr2][valid_rows] == val2)
                    ]

                    if len(pattern_rows) >= min_rows:
                        if isCurNodeFD_InRows(encoder_data, col_dict, rhs_col_name, has_data_index, lhs_indices, pattern_rows):
                            # Build pattern: specific values for these two LHS attrs
                            pattern = ['_'] * (len(lhs_indices) + 1)
                            pattern[i] = str(val1)
                            pattern[j] = str(val2)
                            valid_patterns.append((pattern, len(pattern_rows)))

    return valid_patterns if valid_patterns else None


def enumerate_patterns_for_cfd(lhs_indices, rhs_index, encoder_data, col_dict, has_data_index, miss_data, min_rows):
    """
    Enumerate patterns for CFD mining - returns all valid patterns and their support.
    """
    return enumerate_patterns_for_cfd_simple(lhs_indices, rhs_index, encoder_data, col_dict, has_data_index,
                                           np.arange(len(miss_data)), min_rows)


def updateFeatLabels(new_node, featLabels):
    #Check if featLabels has its subset
    node_list = []
    for node in featLabels:
        if set(new_node) <= set(node) and len(new_node) > 0:
            node_list.append(node)
    if len(node_list) > 0:
        # print(f"  Remove redundant FD: {[n for n in node_list]} ( by new FD {new_node}  included)")
        for n in node_list:
            featLabels.remove(n)
...
def isCurNodeFD(encoder_data, col_dict, col_name, has_data_index, cur_node):
    ...
    # Check if data partitioned by left side is consistent in the right side partition
...
def update_FD_models(generate_x, zero_feed_data, fields, data_m, M_tensor, value_cat, values, miss_data_x, enc, device, eq_dict, FDs_model_list, cell_acc, continuous_cols, cost_time):
    ...
    for FD in FDs_model_list:
        ...
        # if y_index == 5:
        #     print(1)
        FD_score, un_sat_score, un_satisfy_tup, all_satisfy_tup_inS, all_satisfy_tup_inS_acc = get_FD_score(satisfy_tuples, un_satisfy_tuples, tuple_acc_list)  # [0, 1], 0 means all unsatisfied, 1 means all satisfied
...
# CFD-related classes and function implementations
class CFDModel(nn.Module):
    def __init__(self, input_size, output_size, lhs, rhs, pattern):
        ...
        self.fc2 = nn.Linear(input_size, input_size)
        self.fc3 = nn.Linear(input_size, output_size)
        self.lhs = lhs  # LHS attribute index list
        self.rhs = rhs  # RHS attribute index
        self.pattern = pattern  # pattern list, containing constants and '_'
...
def calculate_cfd_score(lhs, rhs, pattern, impute_data, eq_dict, tuple_acc_list):
    """
    Calculate CFD score: reuse FD score calculation logic on pattern subset
    """
    # Get tuples satisfying pattern, as RES_X
    RES_X = get_pattern_satisfying_tuples(lhs + [rhs], pattern, impute_data, eq_dict)

    if len(RES_X) == 0:
        return 0.0, 0.0, '', [], []

    # Reuse FD satisfy/unsatisfy classification logic on pattern subset
    satisfy_tuples, un_satisfy_tuples = get_satisfy_unsatisfy_tuples(RES_X, eq_dict, rhs, impute_data, tuple_acc_list)

    # Directly use FD score calculation function
...
def update_CFD_models(generate_x, zero_feed_data, fields, data_m, M_tensor, value_cat, values, miss_data_x, enc, device, eq_dict, CFD_model_list, cell_acc, continuous_cols, cost_time, dict_A, dict_B, dict_cnt, min_rows, max_pattern_per_structure):
    """
    CFD refine operation: refine current CFD list
    Note: top_k will dynamically adjust based on dict_B changes
    """
    x_code = generate_x * (1 - M_tensor) + M_tensor * zero_feed_data
    new_x_code = x_code.clone()
    impute_data = reconvert_data(x_code, fields, value_cat, values, miss_data_x, data_m, enc)
    ...
    for CFD in CFD_model_list:
        ...
        # Calculate attribute index list for this CFD (LHS + RHS)
        index_list = lhs.copy()
        index_list.append(rhs)

        # Calculate tuple_acc_list: use pattern-aware cell_acc
        tuple_acc_list = []
        for tuple_index, tuple_acc in enumerate(cell_acc):
            tuple_acc_list.append(min(tuple_acc[index_list]))

        # Step 1: Calculate CFD score within current pattern domain
        ...
        # Step 2: If CFD_score - max_un_sat_score < 0, perform refine (similar to FD judgment logic)
        if CFD_score - max_un_sat_score < 0:
            structure_key = (tuple(lhs), rhs)

            # Step 2.1: Query dict_cnt[(LHS, RHS)]
            if dict_cnt.get(structure_key, 0) > max_pattern_per_structure:
                # Remove current CFD from dict_A and CFD list (terminal state)
                if structure_key in dict_A:
                    del dict_A[structure_key]
                new_CFD_model_list.remove(CFD)

                # Enumerate top-K patterns from dict_B[(LHS, RHS)] sorted by support descending
                if structure_key in dict_B:
                    success = False

                    # Calculate number of candidate patterns satisfying support >= min_rows/2 for this structure as current top_k
                    ...
                    for i, (cand_pattern, support) in enumerate(candidate_patterns):
                        # Calculate CFD score for candidate pattern
                        ...
                        if support >= min_rows and cand_CFD_score > CFD_score:
                            # Remove this pattern from dict_B
                    ...
                    # If all top-K fail, directly end refine for this structure
                    if not success:
                        continue
                else:
                    continue

            # Step 2.2: If structure count does not exceed threshold, perform specialization on current pattern
            else:
                specialized = False

                # Attempt pattern specialization: replace '_' with concrete attribute value
                ...
                # Step 2.3: If pattern specialization fails, perform LHS augmentation
                if not specialized:
                    augmented = False

                    # Gradually add new attributes to LHS, new attribute corresponding pattern starts enumeration from '_'
                    ...
                    # If all LHS extensions fail, remove this CFD (terminal state)
                    if not augmented:
                        structure_key = (tuple(lhs), rhs)
                        if structure_key in dict_A:
                            del dict_A[structure_key]
        ...
        # Step 3: If c(φ) ≥ c(φ*), consider this CFD valid, update corresponding pattern in dict_A
        else:
            structure_key = (tuple(lhs), rhs)
...
def calculate_cfd_confidence(lhs, rhs, pattern, impute_data, eq_dict, cell_acc):
    """
    Calculate CFD confidence, only on subtable corresponding to pattern condition
    """
    # Get tuples satisfying pattern
    pattern_tuples = get_pattern_satisfying_tuples(lhs + [rhs], pattern, impute_data, eq_dict)

    if len(pattern_tuples) == 0:
        return 0.0

    # Calculate number of tuples satisfying FD (same LHS and RHS), weighted by cell_acc
    satisfy_score = 0.0
    total_score = 0.0

    for tuples in pattern_tuples:
        if len(tuples) > 1:
            # Check if LHS is same and RHS is same
            ...
            # Check if all LHS are same and all RHS are same
            ...
            # Use cell_acc to weight score for each tuple
...
def get_pattern_satisfying_tuples(attrs, pattern, impute_data, eq_dict):
    """
    Get tuple groups satisfying pattern
    """
    pattern_tuples = []

    # Start grouping from smallest equivalence class
    min_attr = min(attrs)
...
def check_pattern_match(tuples, attrs, pattern, impute_data):
    """
    Check if tuple group satisfies pattern
    """
    if len(tuples) == 0:
        return False
    ...
    for i, attr in enumerate(attrs):
        if pattern[i] != '_':
            # Check if all tuples have the same value as pattern[i] for this attribute
...
def recalculate_cfd_metrics(lhs, rhs, pattern, impute_data, eq_dict, cell_acc):
    """
    Recalculate CFD support and confidence
    """
    pattern_tuples = get_pattern_satisfying_tuples(lhs + [rhs], pattern, impute_data, eq_dict)

    # Use cell_acc to calculate weighted support
    support = 0.0
    for tuples in pattern_tuples:
        # For each tuple group, weight with cell_acc
        tuple_score = sum(cell_acc[tuple_idx, attr] for tuple_idx in tuples for attr in lhs + [rhs]) / len(tuples) / len(lhs + [rhs])
        support += tuple_score
...
def pattern_specialization(lhs, rhs, pattern, impute_data, eq_dict, cell_acc, tuple_acc_list, min_rows):
    """
    Pattern specialization: replace '_' with concrete attribute value
    """
    best_pattern = None
    best_CFD_score = -1.0
    ...
    for i, val in enumerate(pattern):
        if val == '_':
            ...
            # Attempt to replace '_' with concrete value for this attribute
            ...
            for concrete_val in unique_vals:
                ...
                # Calculate new pattern support and CFD score
...
def lhs_augmentation(lhs, rhs, pattern, impute_data, eq_dict, cell_acc, tuple_acc_list, values, continuous_cols, min_rows):
    """
    LHS augmentation: gradually add new attributes to LHS
    """
    best_lhs = None
    best_pattern = None
    best_CFD_score = -1.0

    # Try adding new attribute to LHS
    for attr_idx in range(len(values)):
        if attr_idx not in lhs and attr_idx != rhs and attr_idx not in continuous_cols:
            ...
            new_pattern = pattern + ['_']  # New attribute corresponding pattern starts from '_'

            # Calculate new LHS CFD score
