...
class Network(nn.Module):
    ...
    def predict(self, x, y):
        ...
        if self.cfg.prototypes.k > 0:
            a = self.ProtoEmbedIdx.type_as(y)
            prototypes = self.ProtoEmbed(a)
    ...
    def loss_function(self, batch, mode='train'):
        ...
        x_new = batch.x_clean * batch.mask
        output = self.calculate_imputation_loss(x_clean=x_new,
                                                num_rec=num_rec, 
    ...
    def calculate_imputation_loss(self, x_clean, num_rec, cat_outputs, mask, cat_idx, num_idx):
        ...
        if len(num_idx) > 0:
            ...
            x_num = x_clean[:, num_idx][num_corrupted_idxs]
            # x_fill = x[:, num_idx][num_corrupted_idxs]
            # Such indexing gives vectors with size: (n_corrupted_values, 1)
