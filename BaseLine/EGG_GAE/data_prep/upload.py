...
def loader(**kwargs):
    ...
    elif kwargs['dataset'] == 'balance':
        data = pd.read_csv(f'{dir_path}/data/balance/balance.csv')
        y = np.array(data['clase'])
        X = np.array(data.drop(columns=['clase']))

    elif kwargs['dataset'] == 'breast':
        data = pd.read_csv(f'{dir_path}/data/breast/breast.csv')
        y = np.array(data['clase'])
        X = np.array(data.drop(columns=['clase']))
    ...
    args = {
        ...
        # ["SUSY-small", "eeg", "yeast", "wireless", "CalHousing", "red_wine", "abalone"]
            ...
            "adult": { 'dataset':"adult",
                        ...
                        'num_classes': 2,  # for classification
                        'num_features': 14,
                        'cat_idx': [1,3,4,5,6,7,8,9,13],
                        # cat_dims: will be automatically set.
                        'cat_dims': [9, 16, 7, 7, 15, 6, 5, 2, 42],
                        'objective':'classification',
                        'leave_only_numerical': False
                        }
